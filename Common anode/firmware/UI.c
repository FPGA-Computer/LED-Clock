...
void UI_Print_HM(uint8_t Hour, uint8_t Min, const uint8_t display_opt)
{
	UI_Print_Hr(Hour,display_opt);
		
	Display[HR_COL+1]|=SEP_SEG;
	UI_Print2d(Min,MIN_COL);
}

void UI_Print_Time(rtc_t *time,const uint8_t display_opt)
{
	UI_Print_HM(time->hour,time->min,display_opt);
...
#ifdef ALARM
const uint8_t AL_Status[] = { Ch_Neg,Ch_Neg,0,Ch_A,Ch_L,0 };
#endif	
...
#ifdef ALARM				
			case D_AL:
				UI_Print_Str(&AL_Status[*(uint8_t*)Item->Value*3],x);
				break;
#endif
